...
import { Plus, Search, Users, X, Eye, Pencil, Trash2, Star, MapPin, BookOpen, DollarSign, CheckCircle } from 'lucide-react';
...
function ToastContainer({ toasts, onDismiss }: { toasts: ToastMessage[]; onDismiss: (id: number) => void }) {
  ...
  return (
    <div className="fixed top-4 right-4 z-[60] flex flex-col gap-2">
      {toasts.map((t) => (
        <div key={t.id} className={`flex items-center gap-3 rounded-lg px-4 py-3 shadow-lg border text-sm font-medium min-w-[280px] ${t.type === 'success' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
          {t.type === 'success' ? <CheckCircle size={16} className="text-green-600 shrink-0" /> : <X size={16} className="text-red-600 shrink-0" />}
...
/* ─── Card ─── */
function TutorCard({ tutor, onView, onEdit, onDelete }: { tutor: Tutor; onView: (t: Tutor) => void; onEdit: (t: Tutor) => void; onDelete: (t: Tutor) => void }) {
  return (
    <div className="group relative rounded-xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition-all">
      <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onView(tutor)} className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100" title="View"><Eye size={15} /></button>
        <button onClick={() => onEdit(tutor)} className="rounded-md p-1.5 text-slate-500 hover:bg-blue-50 hover:text-blue-600" title="Edit"><Pencil size={15} /></button>
        <button onClick={() => onDelete(tutor)} className="rounded-md p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-600" title="Delete"><Trash2 size={15} /></button>
      </div>

      <div className="flex items-start gap-3">
        <img src={tutor.avatar || 'https://via.placeholder.com/64'} alt={tutor.name} className="h-14 w-14 rounded-lg object-cover border border-slate-100" />
        <div className="min-w-0 flex-1 pr-16">
          <div className="flex items-center gap-1.5">
            <h3 className="text-sm font-bold text-slate-900 truncate">{tutor.name}</h3>
            {tutor.verified && <CheckCircle size={13} className="text-blue-500 shrink-0" />}
          </div>
          <p className="text-xs text-slate-500 truncate">{tutor.title}</p>
          <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-1"><MapPin size={11} />{tutor.country} {tutor.countryFlag}</p>
        </div>
      </div>

      <div className="mt-3 flex items-center gap-3 text-xs text-slate-600">
        <span className="flex items-center gap-1 text-amber-600 font-medium"><Star size={12} fill="currentColor" />{tutor.rating}</span>
      ...
      <div className="mt-2 flex flex-wrap gap-1">
        {tutor.subjects.slice(0, 3).map(s => (
          <span key={s} className="inline-flex items-center gap-1 rounded bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium text-brand-700"><BookOpen size={9} />{s}</span>
        ))}
        {tutor.subjects.length > 3 && <span className="text-[10px] text-slate-400">+{tutor.subjects.length - 3}</span>}
      </div>

      <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2">
        <span className="text-xs font-semibold text-slate-800 flex items-center gap-0.5"><DollarSign size={12} />{tutor.pricePerHour}<span className="text-slate-400 font-normal">/hr</span></span>
        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${tutor.available ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
...
function Modal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-2xl">{children}</div>
    </div>
  );
}
    ...
    <Modal onClose={onClose}>
      <form onSubmit={handleSubmit} className="p-5">
        ...
        <div className="space-y-3">
          ...
          <div><label className={label}>Subjects (comma separated)</label><input className={input} value={form.subjects} onChange={e => set('subjects', e.target.value)} placeholder="Math, Physics, Chemistry" /></div>
    ...
    <Modal onClose={onClose}>
      <div className="p-5">
        ...
        <div className="flex items-start gap-3 mb-4">
          <img src={tutor.avatar || 'https://via.placeholder.com/80'} alt="" className="h-16 w-16 rounded-xl object-cover border" />
          <div>
            ...
            <p className="text-xs text-slate-400">{tutor.country} {tutor.countryFlag}</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="rounded-lg bg-slate-50 p-2 text-center text-xs"><span className="block font-bold text-slate-900">{tutor.rating} ★</span>Rating</div>
          <div className="rounded-lg bg-slate-50 p-2 text-center text-xs"><span className="block font-bold text-slate-900">{tutor.students}</span>Students</div>
          <div className="rounded-lg bg-slate-50 p-2 text-center text-xs"><span className="block font-bold text-slate-900">{tutor.hoursTaught}h</span>Taught</div>
        </div>
        <div className="space-y-2 text-xs text-slate-600">
          <p><span className="font-semibold">Bio:</span> {tutor.bio || '—'}</p>
          ...
          <p><span className="font-semibold">Price:</span> ${tutor.pricePerHour}/hr</p>
          <p><span className="font-semibold">Status:</span> {tutor.available ? 'Available' : 'Unavailable'} | {tutor.verified ? 'Verified' : 'Not Verified'}</p>
        </div>
      </div>
    </Modal>
  );
...
/* ─── Delete Modal ─── */
function DeleteModal({ tutor, onConfirm, onClose, isDeleting }: { tutor: Tutor; onConfirm: () => void; onClose: () => void; isDeleting: boolean }) {
  return (
    <Modal onClose={onClose}>
      <div className="p-5 text-center">
        <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-red-100"><Trash2 size={20} className="text-red-600" /></div>
        <h2 className="text-base font-bold text-slate-900">Delete Tutor?</h2>
        <p className="mt-1 text-xs text-slate-500">Delete <span className="font-semibold">{tutor.name}</span>? This cannot be undone.</p>
        <div className="mt-4 flex justify-center gap-2">
          <button onClick={onClose} className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
          <button onClick={onConfirm} disabled={isDeleting} className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-60">
...
export default function AdminTutors() {
  ...
  const filtered = useMemo(() => {
    ...
    return tutors.filter(t =>
      [t.name, t.email, t.title, t.country, t.bio].some(f => f?.toLowerCase().includes(term))
    );
  }, [tutors, search]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 mt-14">
      ...
      {modal.type === 'create' && <TutorForm mode="create" onSubmit={handleCreate} onClose={close} isSubmitting={submitting} />}
      {modal.type === 'edit' && <TutorForm mode="edit" tutor={modal.tutor} onSubmit={(d) => handleUpdate(modal.tutor._id, d)} onClose={close} isSubmitting={submitting} />}
      {modal.type === 'view' && <ViewModal tutor={modal.tutor} onClose={close} />}
      {modal.type === 'delete' && <DeleteModal tutor={modal.tutor} onConfirm={() => handleDelete(modal.tutor)} onClose={close} isDeleting={deleting} />}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={() => setModal({ type: 'create' })} className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-black bg-amber-800 shadow-sm hover:bg-brand-700 transition-all">
        ...
        <div className="mt-8">
          {loading ? (
            ...
            <div className="flex flex-col items-center gap-3 rounded-2xl border border-red-100 bg-red-50 py-16 text-center">
              ...
              <button onClick={load} className="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">Try again</button>
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-slate-200 bg-white py-24 text-center">
              ...
              <p className="text-sm font-medium text-slate-500">{tutors.length === 0 ? 'No tutors yet' : 'No matches'}</p>
            </div>
          ) : (
