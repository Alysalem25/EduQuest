...
export default function FindTutor() {
  ...
  return (
    <div className="bg-eqraa-beige-light pt-28">
      ...
      {/* Main: Form + Results side by side */}
      <section className="pb-12">
        <div className="container-px flex flex-col lg:flex-row gap-8 items-start">
          {/* Left: Search Form */}
          <div className="w-full lg:w-[420px] shrink-0">
            <div className="rounded-[2rem] bg-white p-6 shadow-soft sm:p-8">
              {/* Step header */}
              <div className="mb-5 flex items-center gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl gradient-brown text-sm font-bold text-white">
                  {activeStep + 1}
                </span>
                <div>
              ...
              {activeStep === 0 && (
                <div className="animate-fade-in">
                  ...
                  <div className="flex max-h-[260px] flex-wrap content-start gap-3 overflow-y-auto rounded-2xl p-1">
                    {filteredSubjects.map((subject) => {
                      ...
                      return (
                        <button
                          key={subject.id}
                          type="button"
                          onClick={() => {setSelectedSubject(subject), handleSearch}}
                          className={`flex flex-1 basis-[120px] flex-col items-center gap-2 rounded-3xl border-2 p-4 text-center transition-all duration-300 min-w-[120px] ${
                            selected
                    ...
                    {filteredSubjects.length === 0 && (
                      <p className="w-full py-8 text-center text-sm text-eqraa-brown-dark/50">
              ...
              {activeStep === 1 && (
                <div className="flex max-h-[260px] animate-fade-in flex-col gap-3 overflow-y-auto rounded-2xl p-1">
                  {educationLevels.map((level) => {
                    ...
                    return (
                      <button
                        key={level.id}
                        type="button"
                        onClick={() => {setSelectedLevel(level) ,handleSearch}}
                        className={`flex items-center gap-4 rounded-3xl border-2 p-5 text-left transition-all duration-300 ${
                          selected
                            ? "border-eqraa-brown bg-eqraa-beige shadow-soft"
                            : "border-eqraa-beige/70 bg-eqraa-beige-light hover:-translate-y-0.5 hover:border-eqraa-brown/40 hover:bg-white hover:shadow-soft"
                        }`}
                      >
                        <div
                        ...
                        {selected && (
                          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full gradient-brown text-white">
                            <Check size={16} />
                          </div>
                        )}
                      </button>
                  ...
                  {curricula.map((curriculum) => {
                    ...
                    return (
                      <button
                        key={curriculum.id}
                        type="button"
                        onClick={() => {setSelectedCurriculum(curriculum) ,handleSearch}}
                        className={`flex items-center gap-4 rounded-3xl border-2 p-5 text-left transition-all duration-300 ${
                          selected
                            ? "border-eqraa-brown bg-eqraa-beige shadow-soft"
                            : "border-eqraa-beige/70 bg-eqraa-beige-light hover:-translate-y-0.5 hover:border-eqraa-brown/40 hover:bg-white hover:shadow-soft"
                      ...
                      >
                        <div
                          className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-lg font-bold transition-colors ${
                            selected
                              ? "gradient-brown text-white"
                              : "bg-white text-eqraa-brown shadow-soft"
                          }`}
              ...
              {(selectedSubject || selectedLevel || selectedCurriculum) && (
                <div className="mt-5 flex flex-wrap gap-2 border-t border-eqraa-beige/70 pt-4">
                  {selectedSubject && (
              ...
              {/* Nav controls */}
              <div className="mt-5 flex items-center justify-between gap-4 border-t border-eqraa-beige/70 pt-4">
                <button
                  type="button"
                  onClick={goBack}
          ...
          {/* Right: Results — 2 per row, scrollable */}
          <div
            id="results-panel"
            className="flex-1 min-w-0 max-h-[calc(100vh-220px)] overflow-y-auto rounded-[2rem] bg-white p-6 shadow-soft sm:p-8"
          >
            {searching && (
            ...
            {!searching && hasSearched && (
              <div className="animate-fade-in">
                ...
                {matchedTutors.length === 0 && (
                  <div className="mt-10 rounded-3xl bg-eqraa-beige-light p-8 text-center">
                    ...
                    <p className="mx-auto mt-2 max-w-md text-sm text-eqraa-brown-dark/60">
                      Try adjusting your subject, level, or curriculum — new
                      tutors join Eqraa every week.
                    </p>
            ...
            {!searching && !hasSearched && (
              <div className="mx-auto max-w-2xl py-12 text-center">
                <div className="rounded-3xl border-2 border-dashed border-eqraa-beige-dark/60 bg-eqraa-beige-light/50 p-10">
                  <Search size={36} className="mx-auto text-eqraa-brown/50" />
                  <h3 className="mt-4 text-lg font-semibold text-eqraa-brown-dark">
                    Your tutor matches will appear here
                  </h3>
                  <p className="mt-2 text-sm text-eqraa-brown-dark/60">
                    Complete the three steps on the left and tap{" "}
